...
path_sample_1 = 'audio_data/iptv_uttr/sample1'
path_sample_2 = 'audio_data/iptv_uttr/sample2'
path_sample_3 = 'audio_data/iptv_uttr/sample3'
...
def umap_vis(utterance_embed, speakers):
...
def main():
    ...
    activities = ["EDA", "USC(1)", "USC(2)", "USC(3)"]
    ...
    if choice == 'EDA':
        ...
        # file_to_be_uploaded = st.file_uploader("테스트할 Audio 파일 불러오기", type="wav")
        ...
        #df_new = pd.DataFrame(list(zip(speakers, wavs)), columns=['speakers', 'wavs'])
        df_new = pd.DataFrame(wavs, columns=['wavs'])
        st.dataframe(df_new)

        btn = st.button("UMAP")

        if btn:
            umap_vis(utterance_embeds, speakers)
        else:
            pass

        st.subheader('Speaker Reocognition')
        st.text('※ 본 Demo APP 에서는 USC를 통해 "군집 개수=6"으로 결정함.')
        ...
        st.write("""
        Unknown input에 대해, 군집 중 어느 곳에 속할지 (or 속하지 않는지) 유사도 계산으로 판단함
        """)
        ...
        # test_wav = ["김나영1", "김나영2", "이윤규_Unk_1", "이윤규_Unk_2", "하미향1_unk", "하미향2_unk"]
        test_wav = ["류성균1", "이윤규1_unk", "하미향1_unk", "엄다은_초등저1"]
        choice = st.selectbox("select a sample", test_wav)

        if choice == '류성균1':
            audio_file = open('./audio_data/iptv_uttr/test1/류성균1.wav', 'rb')
            
            audio_bytes = audio_file.read()
            st.audio(audio_bytes, format='audio/wav')

            test_path = Path("audio_data", "iptv_uttr", "test1", "류성균1.wav")
            test_wav = preprocess_wav(test_ptah)
        ...
        elif choice == '이윤규1_unk':
            audio_file = open(
                './audio_data/iptv_uttr/test1/이윤규1_unk.wav', 'rb')
            audio_bytes = audio_file.read()
            st.audio(audio_bytes, format='audio/wav')

            test_path = Path("audio_data", "iptv_uttr",
                             "test1", "이윤규1_unk.wav")
            ...
            audio_file = open(
                './audio_data/iptv_uttr/test1/하미향1_unk.wav', 'rb')
            audio_bytes = audio_file.read()
            st.audio(audio_bytes, format='audio/wav')
        ...
        elif choice == '엄다은_초등저1':
            audio_file = open(
                './audio_data/iptv_uttr/test1/엄다은_초등저_1.wav', 'rb')
            audio_bytes = audio_file.read()
            st.audio(audio_bytes, format='audio/wav')
        ...
        test_wav = ["김지현1_unk", "이윤규1_unk", "백광제1", "배한슬_초등고1"]
        choice = st.selectbox("select a sample", test_wav)

        if choice == '김지현1_unk':
            audio_file = open(
                './audio_data/iptv_uttr/test2/김지현1_unk.wav', 'rb')
            
            audio_bytes = audio_file.read()
            st.audio(audio_bytes, format='audio/wav')

            test_path = Path("audio_data", "iptv_uttr",
                             "test2", "김지현1_unk.wav")
            test_wav = preprocess_wav(test_path)
            test_embed = encoder.embed_utterance(test_wav)
            ...
            audio_file = open(
                './audio_data/iptv_uttr/test2/이윤규1_unk.wav', 'rb')
            
            audio_bytes = audio_file.read()
            st.audio(audio_bytes, format='audio/wav')

            test_path = Path("audio_data", "iptv_uttr",
                             "test2", "이윤규1_unk.wav")
            test_wav = preprocess_wav(test_path)
            test_embed = encoder.embed_utterance(test_wav)
            ...
            audio_file = open(
                './audio_data/iptv_uttr/test2/백광제1.wav', 'rb')
        ...
        elif choice == '배한슬_초등고1': 
            audio_file = open(
                './audio_data/iptv_uttr/test2/배한슬_초등고_1.wav', 'rb')
            
            audio_bytes = audio_file.read()
            st.audio(audio_bytes, format='audio/wav')

            test_path = Path("audio_data", "iptv_uttr",
                             "test2", "배한슬_초등고_1.wav")
            test_wav = preprocess_wav(test_path)
            test_embed = encoder.embed_utterance(test_wav)
        ...
        test_wav = ["김나영1", "이윤규1_unk", "하미향1_unk", "임서영_초등저1"]
        choice = st.selectbox("select a sample", test_wav)

        if choice == '김나영1':
            audio_file = open('./audio_data/iptv_uttr/test3/김나영1.wav', 'rb')
            
            audio_bytes = audio_file.read()
            st.audio(audio_bytes, format='audio/wav')

            test_path = Path("audio_data", "iptv_uttr", "test3", "김나영1.wav")
            test_wav = preprocess_wav(test_path)
            test_embed = encoder.embed_utterance(test_wav)
            ...
            audio_file = open(
                './audio_data/iptv_uttr/test3/이윤규1_unk.wav', 'rb')
            
            audio_bytes = audio_file.read()
            st.audio(audio_bytes, format='audio/wav')

            test_path = Path("audio_data", "iptv_uttr",
                             "test3", "이윤규1_unk.wav")
            test_wav = preprocess_wav(test_path)
            test_embed = encoder.embed_utterance(test_wav)
            ...
            audio_file = open(
                './audio_data/iptv_uttr/test3/하미향1_unk.wav', 'rb')  
            audio_bytes = audio_file.read()
            st.audio(audio_bytes, format='audio/wav')

            test_path = Path("audio_data", "iptv_uttr",
                             "test3", "하미향1_unk.wav")
            test_wav = preprocess_wav(test_path)
            test_embed = encoder.embed_utterance(test_wav)
        ...
        elif choice == '임서영_초등저1':
            audio_file = open(
                './audio_data/iptv_uttr/test3/임서영_초등저1.wav', 'rb')
            
            audio_bytes = audio_file.read()
            st.audio(audio_bytes, format='audio/wav')

            test_path = Path("audio_data", "iptv_uttr",
                             "test3", "임서영_초등저1.wav")
            test_wav = preprocess_wav(test_path)
            test_embed = encoder.embed_utterance(test_wav)
